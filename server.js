    ...
    add:(printer)=>{
        ...
        mysqlPool.getConnection((err,conn)=>{
            ...
            conn.query(query,(err)=>{
                ...
                if(!err)sockets.emit('entradaEstoque',printer);
            })
        })
    },
}
const users={
    add:(username,callback)=>{
        ...
        mysqlPool.getConnection((err,conn)=>{
            ...
            err=>{
                conn.release()
                if(!err) return callback(NickName);
                return callback()
            })
        })
    },
    nickname:(username,callback=()=>{console.log('.nickname sem callback')})=>{
        mysqlPool.getConnection((err,conn)=>{
    ...
    perms:(username,callback=()=>{console.log('.perms sem callback')})=>{
        mysqlPool.getConnection((err,conn)=>{
            conn.query(`SELECT Permission FROM Users WHERE UserName='`+username+`'`,
            (err,results)=> {
                conn.release()
                if(results[0]) return callback(JSON.parse(results[0].Permission))
                return callback()
            })
        })
    },
    editPerm:(username,newPermission=[])=>{
        newPermission=JSON.stringify(newPermission)
        mysqlPool.getConnection((err,conn)=>{
            conn.query(`UPDATE Users SET Permission='`+newPermission+`' WHERE UserName='`+username+`'`,
            (err)=> {
                conn.release()
                if (!err) return console.log('set permission from '+username+' to '+newPermission)
                return console.log('error in set permission '+username)
            })
        })
    },
}
const session={
    ldapAuth:(username,password,callback)=>{
    ...
    secret: '621f831e68c5a55a4f8e9b58f40b83b8',
    lifespan: 28800,
}
            ...
            mssqlPool.then(pool=>{
                ...
                .then(({recordset})=>{
                    if(recordset[0]){
                        ...
                        console.log('CodVisita: '+CodVisita)
                    ...
                    .then(({recordset})=>{
                        if(recordset[0]){
                            ...
                            console.log('CodMaquina: '+CodMaquina)
                            recordset.forEach(row => {
                                row.Tipo='NOVA'
                                while(row.Serial!=(row.Serial=row.Serial.replace(' \t','')));
                                printer.add(row)
...
/* Routes
----------*/
const motivosRetorno=['Não utilizado pelo Técnico','Retorno Homologação','Retorno Erro de Cadastro']
// Logon
...
const auth=(req,res) => {
    if (!req.body.username) {
        ...
        session.ldapAuth(req.body.username,req.body.password,(token)=>{
            if(token){
...
app.get('/central', (req,res)=>{
    res.redirect('/central/etiquetas')
})
// /Central/Etiquetas
...
// /Central/Estoque
const tecnicos=['Evaldo Borges','Igor Sapucaia','Jeferson Santos','Marcelo Tiago','Marcos Menezes','Paulo Ricardo','Rosangelo Garcez','Weliton Crispim','Diego Sena']
const estoque=(req,res) => {
    res.render('central/estoque',{nickname: req.nickname,tecnicos:tecnicos})
}
app.get('/central/estoque',estoque)
// /Central/saidaEstoque
const saidaEstoque=(req,res) => {
...
app.get('/central/saidaEstoque',saidaEstoque)
// /Central/saidaEstoque
...
sockets.on('connection',socket=>{
    ...
    jwt.verify(uid,session.secret,(err,decoded)=> {
        if(decoded){{
            ...
            printer.possivelMov(result=>{
                socket.emit('movimentacoes',result,motivosRetorno)
            })
            printer.historico(result=>{
                socket.emit('historicos',result)
            })
            socket.on('eventos',request=>{
                printer.eventos(request,result=>{
                    socket.emit('eventos',result)
                })
            })
            socket.on('confirmarSaida',serial=>{
                users.perms(decoded.username,perms=>{
                    if(perms.includes('movimentar')){
                        mysqlPool.getConnection((err,conn)=>{
                            conn.query(`SELECT * FROM PossivelMov WHERE Serial LIKE'%`+serial+`%'`,
                            (err,result)=> {
                                result[0].DataModificacao=moment().format('YYYY-MM-D HH:mm:ss')
                                if(result[0].DataAtendimento)result[0].DataAtendimento=moment(result[0].DataAtendimento).format('YYYY-MM-D HH:mm:ss')
                                if(result[0]){
                                    ...
                                    query+=')VALUES('
                                    query+=`'`+result[0][keys[0]]+`'`
                                    for(let i=1;i<keys.length;i++){
                                        if(result[0][keys[i]]){
                                            query+=`,'`+result[0][keys[i]]+`'`
                                        }else{
                                            query+=`,`+result[0][keys[i]]
                                        }
                                    }
                                    ...
                                    conn.query(query,err=>{
                                        if(!err){
                                            conn.query(`DELETE FROM PossivelMov WHERE Serial LIKE'%`+serial+`%'`,err=>{
                                                sockets.emit('confirmarSaida',serial)
                                                conn.release()
                                            })
                                        }else{
                                            conn.release()
            ...
            socket.on('salvar',printer=>{
                users.perms(decoded.username,perms=>{
                    if(perms.includes('movimentar')){
                        mysqlPool.getConnection((err,conn)=>{
                            conn.query(`SELECT Fonte FROM PossivelMov WHERE Serial LIKE'%`+printer.serial+`%'`,(err,result)=> {
                                if(!err&&result[0].Fonte!='SISTEMA'){
                                    let query='UPDATE PossivelMov SET'
                                    query+=` tipoMovimentacao=`
                                    if(printer.tipoMov.length!=0){
                                        query+=`'`+printer.tipoMov+`'`
                                    }else{
                                        query+='NULL'
                                    }
                                    query+=`,CodSolicitacao=`
                                    if(printer.numOS.length!=0){
                                        query+=`'`+printer.numOS+`'`
                                    }else{
                                        query+='NULL'
                                    }
                                    ...
                                    if(printer.equipRetirado.length!=0){
                                        query+=`'`+printer.equipRetirado+`'`
                                    }else{
                                        query+='NULL'
                                    }
                                    ...
                                    if(printer.contPBRetirado.length==0){
                                        printer.contPBRetirado='0'
                                    }
                                    query+=`,ContadorPB='`+printer.contPBRetirado+`'`
                                    if(printer.contColorRetirado.length==0){
                                        printer.contColorRetirado='0'
                                    }
                                    query+=`,ContadorColor='`+printer.contColorRetirado+`'`
                                    query+=`,Localizacao=`
                                    ...
                                    if(printer.pendLeitura.length!=0){
                                        query+=`'`+printer.pendLeitura+`'`
                                    }else{
                                        query+='NULL'
                                    }
                                    ...
                                    query+=`,Fonte='`+decoded.username+`'`
                                    query+=` WHERE Serial LIKE '%`+printer.serial+`%'`
                                    conn.query(query,(err)=> {
                                        if(!err)sockets.emit('salvar',printer)
                                    })
                                }
                            })
                        })
